...
const PREFIX_URL = '/static/images/';
...
class App extends React.Component {
  ...
  constructor(props) {
    ...
    this.images = [
      {
        isVideo: true,
        thumbnail: `${PREFIX_URL}01.webp`,
        original: `${PREFIX_URL}01.webp`,
        file: `${PREFIX_URL}01.mp4`,
        description: 'Render custom slides (such as videos)',
        renderItem: this._renderVideo.bind(this)
      },
      {
        original: `${PREFIX_URL}02.webp`,
        thumbnail: `${PREFIX_URL}02.webp`,
  ...
  _getStaticImages() {
    ...
    for (let i = 2; i < 6; i++) {
      images.push({
        original: `${PREFIX_URL}0${i}.webp`,
        thumbnail:`${PREFIX_URL}0${i}-thumb.webp`
      });
    }
  ...
  _renderVideo(item) {
    return (
      <div className='video-placeholder-wrapper'>
        {
          this.state.showVideo[item.file] ?
            <div className='video-wrapper'>

                <a
                  className='close-video'
                  onClick={this._toggleShowVideo.bind(this, item.file)}
                >
                </a>
                <video src={item.file} loop={true} autoPlay={true} />
                {/*<iframe*/}
                {/*  width='560'*/}
                {/*  height='315'*/}
                {/*  src={item.embedUrl}*/}
          ...
          :
            <a onClick={this._toggleShowVideo.bind(this, item.file)}>
              <div className='play-button'></div>
              <img alt={item.title} className='image-gallery-image' src={item.original} />
              {
                  item.description &&
                  <span
